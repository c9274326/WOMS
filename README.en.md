...
- Scheduled orders can be moved into production from the order list or by clicking the order on the calendar. Starting production locks all allocations for that order. In-progress orders can be confirmed from the order list or calendar as fully complete or partially complete; partial completion records the produced quantity in workflow history and returns the same order ID to pending scheduling with the remaining quantity.
