...
- Scheduler users can preview selected pending orders first, or drag one pending order directly onto a calendar day. A drag drop creates the schedule immediately when there are no conflicts; conflicting drops open the conflict-resolution page instead. Scheduling starts from the selected start date and fills the earliest available capacity first. When conflicts exist, the preview page shows the allocation plan, a suggested earlier start date, per-order due-date edits with retry, and manual-force retry when allowed. Manual intervention requires a reason and explicit conflict acknowledgements before the job is accepted. Direct schedule-job creation without `previewId` is rejected.
...
- Scheduled orders can be moved into production. Starting production locks all allocations for that order. In-progress orders can be confirmed from a dedicated production modal as fully complete or partially complete; partial completion closes the produced quantity and creates a pending child order for the remainder.
