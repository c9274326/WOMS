...
- Scheduler users can preview selected pending orders first, or drag pending orders onto any visible non-past calendar day. Drag scheduling now treats the dropped calendar day as the requested schedule date, so dropping a future-due order on May 2 previews and persists the allocation on May 2 when capacity is available. When conflicts exist, the preview page shows the allocation plan, a suggested earlier start date, per-order due-date edits with retry, and manual-force retry when allowed. Manual intervention requires a reason and explicit conflict acknowledgements before the job is accepted. Direct schedule-job creation without `previewId` is rejected.
