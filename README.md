...
- Exact filters support customer and priority. Customer filtering uses chip buttons, while order status is controlled by the left status panel.
...
- Sales users can add customer orders to pending scheduling only; draft feasibility is checked against existing scheduled allocations, not all other pending orders. Order notes are write-on-create only; rejected-order resubmission can adjust due date and quantity but cannot rewrite the original note.
- Scheduler users can preview selected pending orders first, or drag pending orders onto any visible non-past calendar day. Drag scheduling uses the current day as the fastest start date and fills the earliest available capacity first, so a future-due order can consume today's remaining capacity before using the due date. When conflicts exist, the preview page shows the allocation plan, a suggested earlier start date, per-order due-date edits with retry, and manual-force retry when allowed. Manual intervention requires a reason and explicit conflict acknowledgements before the job is accepted. Direct schedule-job creation without `previewId` is rejected.
...
- Scheduled orders can be moved into production. Starting production locks all allocations for that order. In-progress orders can be confirmed from a dedicated production modal as fully complete or partially complete; partial completion closes the produced quantity and transfers unproduced allocations to a scheduled child order for the remainder.
