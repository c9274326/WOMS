...
- scheduler 必須先預覽已選取的待排程訂單，再從 preview 頁面確認執行。排程會從指定開始日期往後填入最早可用產能，讓排程盡早完成。發生衝突時，preview 頁會轉成衝突處理頁，顯示原因說明，並提供改從今天重新試排、調整開始日期重新試排與允許時的人工強制介入重新試排。人工介入必須填寫原因並逐項確認衝突清單後才會接受任務；缺少 `previewId` 的直接排程 API 會被拒絕。
