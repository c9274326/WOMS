...
- scheduler 可以先預覽已選取的待排程訂單，也可以把待排程訂單拖到任何可見且非過去的月曆日期。拖曳排程會把實際放下的月曆日期當成指定排程日；例如交期 5/3 的訂單拖到 5/2，產能足夠時會預覽並保存到 5/2。發生衝突時，preview 頁會顯示分配計畫、建議提前開始日、單筆訂單交期修改重試，以及允許時的人工強制介入重新試排。人工介入必須填寫原因並逐項確認衝突清單後才會接受任務；缺少 `previewId` 的直接排程 API 會被拒絕。
