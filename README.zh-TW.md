...
- 訂單狀態數字只統計目前選定產線。
- 月曆每日會顯示以 10,000 片為滿水位的產能水位；水位從藍色開始，越接近滿載越偏橘色，滿載時顯示紅色。
- sales 只能加入客戶訂單到待排程；草稿可行性會與既有已排程配置檢查，不會把其他待排程訂單一起試算。
- scheduler 必須先預覽已選取的待排程訂單，再從 preview 頁面確認執行。發生衝突時，preview 頁會轉成衝突處理頁，顯示原因說明，並提供調整開始日期重新試排與人工強制介入重新試排。人工介入必須填寫原因並逐項確認衝突清單後才會接受任務；缺少 `previewId` 的直接排程 API 會被拒絕。
